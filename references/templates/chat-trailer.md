...
| `implement-cohesively` | yes (4-vocabulary) | `**Thesis:**` | `## Code matches locked design` slot — leads with `**Code matches locked design:** ✓` (Implemented verdict) or `**Drift detected:** ✗ <count> places` (Coverage Drift / Substrate Drift verdicts), synthesized AND-shape from the end-of-run dual reviewer dispatch (`delta-coverage-reviewer` returning Covered/Drift/Incomplete + `cohesive:review-diff` returning Pass/Pass with notes/Needs substrate/Risky/Block) — followed by `## End-of-run review` block surfacing each reviewer's verdict with a path to its persisted output (`docs/cohesive/reviews/<YYYY-MM-DD>-<slug>-coverage-review.md` and `docs/cohesive/reviews/<YYYY-MM-DD>-<slug>-final-substrate-review.md`), and `## Branch state` (branch + implementation commit count + plan path). Long-form review detail lives in the persisted reviewer output files, not chat |
