...
- **Where it lives:** `scripts/<check-name>.{sh,py}` (or repo-native location); spec lives inline in the convention doc that names the rule.
...
- **Where it lives:** Repo-native location for specs.
...
- **Promotion criteria:** A convention promotes to a named invariant when (a) the wording stabilizes for ≥2 release cycles, (b) a real regression has occurred that the convention was meant to prevent, and (c) a structural enforcement (test/linter/CI check) is feasible.
