...
**Substrate-note as user override.** The default for `Approved + Low` is close-inline (the lower-friction path). When the user judges that a finding has cross-pass relevance — future reviewers should see it as deferred substrate, not silently repaired — the user overrides the close-inline default and substrate-notes the finding in the validation review file's `## Deferred findings (substrate-note overrides)` section with a stable ID, rationale, and a self-citation to the review pass that surfaced it. A user-elected substrate-note that does not land in the validation review file is a substrate violation — the next reviewer can't see it, and the same gap surfaces again in a future pass. The override does not change the agent's rendered Disposition phrase; the agent renders the rule's default, and the user's override is a post-render move.

**User override.** The user can override the rule's recommendation ("just merge — I don't care about the Medium", or substrate-note the Low instead of close-inline). The override is a deliberate move against a published default, not a derivation from a menu. Overrides do not require general persisted annotation in v0.1, *except* substrate-note overrides per the section above (which land in the validation review file's `## Deferred findings (substrate-note overrides)` section). The cost of unannotated overrides is observability: the team cannot count silent overrides per release cycle. Re-evaluation trigger: if more than 3 `validate-rewrite` passes in a single release cycle reveal the same finding repeatedly because it was silently overridden, promote a first-class override-residue surface (a `## Overrides applied` section in the validation review file) in the next pass and update this clause.
