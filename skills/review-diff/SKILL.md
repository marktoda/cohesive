...
- **Invocation contexts (four):** PR review (`gh pr diff <number>`); branch review (`git diff main...HEAD`); working-changes review (`git diff HEAD`); **post-implementation review against a Cohesive-locked design** — given a branch where code landed via a non-Cohesive path (the validate-rewrite Approved-trailer bypass option, a teammate writing code, or an external tool) and a `design/<slug>` rewrite the branch claims to implement, this skill is the verification entry point. Pass the validate-rewrite Approved review path explicitly in the invocation (it carries the rewrite-tip SHA) so the substrate-alignment-reviewer can compute the spec diff and read the locked design as a primary input alongside discovery.
