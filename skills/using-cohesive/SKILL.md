...
5. **The frontmatter description is load-bearing.** The substrate-vocabulary tokens (substrate, cohesion, invariant, gotcha, behavior matrix, spec, rewrite) and the absence of bare generic-review triggers (no "review the codebase", "review the architecture", "is this codebase healthy", "review the code") are what `scripts/validate_plugin.sh` Check 9a/9b enforce, and what makes the harness pick the right plugin at trigger-match time. Drift in the description re-opens the seam.
