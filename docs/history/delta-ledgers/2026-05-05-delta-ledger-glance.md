...
- Pass-2 review New-I2: the across-verdicts render rule ("the `## Delta at a glance` section appears across all three verdicts — Approved / Issues Found / Design Incoherent") lives only in `skills/validate-rewrite/SKILL.md` Output format render-template prose. Single surface today, not yet drift, but unanchored against the agent's render template (`agents/spec-cohesion-reviewer.md`) which carries the same shape without the across-verdicts prose. Deferred because (a) the rule is operationally specific to the validate-rewrite consumer and lives correctly there for now, and (b) promotion would expand `references/templates/cohesion-review.md`'s scope from "review-document shape" to "render-time scoping rule," which is a substrate decision worth making explicitly when a second consumer of the across-verdicts scoping appears. Pass-3 review reaffirmed this deferral with the same rationale; promotion target is `references/templates/design-delta-ledger.md` §"Consumer rendering rules" as a fourth bullet when a second consumer arrives.
- Pass-3 review I1: pre-existing drift between `agents/spec-cohesion-reviewer.md` §"Issue format (canonical six-field shape)" (which requires Severity / Category / Why it matters / Evidence / Recommended fix / Substrate artifact) and `references/templates/cohesion-review.md` issue-shape examples B1/I1 (which still show the older three-field shape: Risk / Substrate artifact / Suggested repair). The agent body line 87 directs reviewers to "Use the template at `references/templates/cohesion-review.md`" — so a literal reading of the template produces the three-field shape and violates the six-field rubric. The contradiction predates this rewrite (the agent's six-field block and the template's three-field examples have been out of sync on `main`), but the new agent line-82 render block makes it newly load-bearing. Deferred from this rewrite because: (a) the issue is unrelated to the delta-glance goal and addressing it here would be scope creep beyond the brainstorm-approved direction; (b) the active spec-cohesion-reviewer dispatches in passes 1–3 of this very rewrite produced the six-field shape, demonstrating that the agent's rubric is being followed in practice over the template's older examples; (c) the fix is small (~6 lines tightening cohesion-review.md B1/I1 to cite `docs/substrate/designs/reviewer-agent-template.md` §"Output format conventions") and is the natural target of a separate `cohesive:rewrite-specs` pass on issue-shape harmonization. A future contributor reading this ledger sees the deferral, the rationale, and the fix shape.
...
Triggered by the pass-2 validate-rewrite review (Approved verdict with two Important issues; persisted at `docs/history/reviews/2026-05-05-delta-ledger-glance-rewrite-validation-pass-2.md`). The pass-2 reviewer flagged that pass-1 repair I1 closed the *category-list* drift but did not close the *consumer rendering rules* drift — the missing-preamble and divergence handling rules still appeared in three surfaces (cohesion-review.md, agent item 11, validate-rewrite render template). Repair scope:

- **Pass-2 New-I1 (Spec drift, Medium) — addressed.** Promoted `references/templates/design-delta-ledger.md` §"Delta at a glance" from "canonical home of category list" to "canonical home of category list + authoring rules + consumer rendering rules" — added an explicit `### Consumer rendering rules` subsection covering the three render-time cases (preamble present and consistent; preamble missing; preamble present but inconsistent with body). Collapsed `references/templates/cohesion-review.md` §"Delta at a glance" to a one-line cite. Tightened `agents/spec-cohesion-reviewer.md` item 11 to cite canonical rendering rules rather than restate them, and tightened the agent's render template to cite as well. Tightened `skills/validate-rewrite/SKILL.md` Output format render-template prose to cite canonical rules.
- **Pass-2 New-I2 (Spec drift, Low) — deferred.** Substrate-noted in §"Remaining ambiguity" above. The across-verdicts render rule lives correctly in `skills/validate-rewrite/SKILL.md` for now (single surface, operationally specific to that consumer); promotion to `references/templates/cohesion-review.md` happens when a second consumer of across-verdicts scoping appears.

Files modified in repair pass 2: `references/templates/design-delta-ledger.md` (§"Delta at a glance" expanded with `### Authoring rules` and `### Consumer rendering rules` subsections; canonical-home claim explicit); `references/templates/cohesion-review.md` (§"Delta at a glance" collapsed to one-line cite); `agents/spec-cohesion-reviewer.md` (item 11 tightened; render template tightened); `skills/validate-rewrite/SKILL.md` (render-template prose tightened to cite canonical rendering rules). §"Files rewritten" body counts unchanged — repair pass 2 tightened citations within the same files already enumerated.

## Repair pass 3 (deferral-only)

Triggered by the pass-3 validate-rewrite review (Approved verdict with two Important issues; persisted at `docs/history/reviews/2026-05-05-delta-ledger-glance-rewrite-validation-pass-3.md`). Pass-3 I1 (Spec drift, Medium) is pre-existing drift between the agent's six-field issue-shape rubric and the cohesion-review.md template's three-field examples. The reviewer correctly framed this as "predates this rewrite but newly load-bearing." Pass-3 I2 (Spec drift, Low) reaffirms pass-2 New-I2's across-verdicts deferral.

Repair scope:

- **Pass-3 I1 — deferred.** Substrate-noted in §"Remaining ambiguity" above. The fix is unrelated to the delta-glance goal and belongs in a separate issue-shape-harmonization rewrite. Three live spec-cohesion-reviewer dispatches in this very rewrite produced the six-field shape from the agent rubric, demonstrating the rubric is the operational source of truth even though the template still shows the older shape. Future contributors who pick up the issue-shape harmonization have the deferred-item entry as their starting point.
- **Pass-3 I2 — deferred (reaffirmed from pass-2 New-I2).** No new action. Promotion target is named in the substrate note for when a second consumer of across-verdicts scoping appears.

No files modified in repair pass 3 beyond this ledger update — the pass is deferral-only. The §"Delta at a glance" preamble counts (5 rewritten, 1 added) remain unchanged because the rewrite's file scope did not change in pass 3.

## Ready for fresh-eyes review?

**Yes** — repair pass 2 closed pass-2 New-I1 (Spec drift, Medium) by promoting the ledger template's §"Delta at a glance" to the full canonical home (category list + authoring rules + consumer rendering rules). Pass-3 I1 (pre-existing drift) and pass-2 New-I2 / pass-3 I2 (single-surface across-verdicts rule) are substrate-noted with deferral rationale and named promotion targets. The rewrite is internally consistent across two repair passes plus one deferral-only pass; the preamble in this very ledger remains the worked example future authors can copy, now with explicit rendering rules in the canonical reference.
