...
**Yes** — repair pass 2 closed pass-2 New-I1 (Spec drift, Medium) by promoting the ledger template's §"Delta at a glance" to the full canonical home (category list + authoring rules + consumer rendering rules). Pass-2 New-I2 is substrate-noted with deferral rationale. The rewrite is internally consistent across both repair passes; the preamble in this very ledger remains the worked example future authors can copy, now with explicit rendering rules in the canonical reference.
