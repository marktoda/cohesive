...
This grid is normative. Adding a route or subskill requires updating this section *and* `${CLAUDE_PLUGIN_ROOT}/skills/cohesively/SKILL.md` §"Dispatch prompt contract" in the same pass. Drift between the two surfaces produces the exact "soft-prereqs" failure mode the contract closes; `scripts/validate_plugin.sh` Check 13i greps both surfaces and asserts route-name set equality plus prereq-state-string parity to catch the drift mechanically.
