...
|---|---|---|---|---|
| `design` | `brainstorm-design` | _none — `brainstorm-design` dispatches `discover-substrate` internally as Step 0 per its Hard constraint #1_ | _none (not yet chosen)_ | Single-skill dispatch; brainstorm-design owns discovery internally so the user sees the brainstorm output, not separate discovery output. rewrite-specs step (only if reached) receives chosen direction; validate-rewrite step (only if reached) receives ledger path. |
| `review (codebase)` | `review-codebase` | _none — `review-codebase` dispatches `discover-substrate` internally as Phase 1.0_ | n/a | Single-skill dispatch. Optional override: if the dispatch prompt names a pre-existing discovery report path, review-codebase reuses it. |
| `review (diff)` | `review-diff` | _none — `review-diff` dispatches `discover-substrate` internally as Step 2 (scoped to changed files)_ | n/a | Single-skill dispatch. Same override semantics. |
| `audit (substrate)` | `audit-substrate` | _none — `audit-substrate` dispatches `discover-substrate` internally as Step 1_ | n/a | Single-skill dispatch. Same override semantics. |
