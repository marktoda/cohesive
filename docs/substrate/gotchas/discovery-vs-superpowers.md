...
- `using-cohesive`'s Hard Constraint #1 ("Orient at most once per session per request shape") and Hard Constraint #4 ("Never re-orient over an already-running route") are reviewer-judged in v0.1 — there is no validator check or runtime guardrail that detects "the user is already inside a Cohesive workflow." The handoff contract for the seam carries no persisted state. A `cohesive:review-diff` reviewer is the canonical surface that flags re-orientation drift, so reviewers of any change touching `using-cohesive`'s body or the seam should explicitly check whether a re-orientation could fire when a route is already in progress; promotion to a structural check waits for a real regression that motivates the wording.
