...
The imperative lives in body prose (in skills, in a `## Voice` section between `## What this skill produces` and `## Hard constraints`; in agents, as the opening prose paragraph of the "How to structure your output" section, above the code block). The model is expected to invoke `Read` on `output-voice.md` before producing chat output — the Read tool call is the loading mechanism; the imperative is what triggers it. Canonical placement for skills is named in `${CLAUDE_PLUGIN_ROOT}/docs/substrate/designs/skill-conventions.md` §"Output format conventions" rule 1; canonical placement for agents is named in `${CLAUDE_PLUGIN_ROOT}/docs/substrate/designs/reviewer-agent-template.md` §"Output format conventions" rule 1, with per-agent compliance tracked at `${CLAUDE_PLUGIN_ROOT}/docs/substrate/matrices/reviewer-output-shape.md`.
