...
- **`implement-cohesively` skill exists and is in the expected skill set** — covered by `scripts/validate_plugin.sh` Check 8 (`expected_skills` array includes `implement-cohesively`) and Check 13f (greps that the skill body cites both `${CLAUDE_PLUGIN_ROOT}/docs/substrate/matrices/phase-derivation.md` and `${CLAUDE_PLUGIN_ROOT}/docs/substrate/invariants/IMPLEMENTATION_PLAN_COVERS_DELTA.md`).
