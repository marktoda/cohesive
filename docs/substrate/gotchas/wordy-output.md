...
1. **Tighten the canonical shape itself.** `docs/substrate/conventions/skill-shape.md` §"Output format conventions" and `docs/substrate/conventions/reviewer-agent-shape.md` §"Output format conventions" cap header depth at `###`, declare that the chat render is substance, not bookkeeping (the faithful-subset test is sub-rule 2a per `${CLAUDE_PLUGIN_ROOT}/references/output-voice.md` rule 2), and require branchy content to render as bullets/tables rather than narrative phases. The shape is the rule; future skill authors copy a tight shape, not a wordy one.
...
- This gotcha and `${CLAUDE_PLUGIN_ROOT}/docs/substrate/gotchas/naming-instead-of-showing.md` are siblings, not duplicates. This one is about *ceremony* (renders are too long, header soup, methodology recap); the sibling is about *substance* (renders are short enough but say nothing concrete — finding-ID references, disposition matrices, empty handoffs). A chat render can fail one without the other; test density and substance independently.
