...
This is one of two named invariants Cohesive ships at v0.1, alongside [`VERDICT_BEFORE_EVIDENCE`](VERDICT_BEFORE_EVIDENCE.md) (every verdict-led skill leads its Output format block with `**Verdict:**`). Other v0.1 rules (chat-render header-depth cap, density budgets, forbidden phrasings, voice-imperative pin (body prose), anti-citation lint (render templates), fresh-eyes preamble, router announcement form, clarifying-question discipline) live as conventions in [`docs/substrate/designs/skill-conventions.md`](../designs/skill-conventions.md), [`docs/substrate/designs/reviewer-agent-template.md`](../designs/reviewer-agent-template.md), and [`references/output-voice.md`](../../../references/output-voice.md). Conventions earn invariant status only when their wording has settled *and* their failure modes are concrete enough to grep for — both bars matter.
