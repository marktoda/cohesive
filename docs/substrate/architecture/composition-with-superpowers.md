...
> Cohesive composes with Superpowers rather than reinventing implementation discipline. Cohesive owns substrate (specs, invariants, gotchas, matrices, reviews) and the substrate-shaped framing that drives implementation: the thin intent paragraph derived from the delta ledger, the delta-size budget gate, and the end-of-run dual reviewer dispatch. Superpowers owns the per-pass plan and TDD execution, plus worktrees and branch finishing. The seam between the two is documented and intentional.
...
| Per-pass plan authoring (thin intent paragraph → TDD plan) | `skills/implement-cohesively/SKILL.md` Step 1 | `superpowers:writing-plans` |
| Per-pass TDD execution (plan → code) | `skills/implement-cohesively/SKILL.md` Step 2 | `superpowers:executing-plans` |
...
Cohesive's worktree path has an inline fallback when Superpowers is absent (a 5-line snippet in `rewrite-specs/SKILL.md`). Cohesive's plan + execution path **does not** have a fallback — `implement-cohesively`'s Hard constraint #2 requires Superpowers. When Superpowers is not installed, `implement-cohesively` stops with a hard error and recommends installation; plan-writing and TDD execution are not 5-line operations and reinventing them inside Cohesive is exactly the duplication the seam exists to prevent.
...
The same logic applies, more sharply, at the implementation seam. `implement-cohesively` orchestrates a single-pass implementation where Cohesive owns *substrate-shape* (the thin intent paragraph derived from the delta ledger; the delta-size budget gate; the end-of-run dual reviewer dispatch combining `delta-coverage-reviewer` + `cohesive:review-diff` with AND-shape verdict synthesis) and Superpowers owns *TDD-shape* (the per-pass plan; TDD execution; branch finishing). Collapsing those two shapes into one Cohesive skill would make Cohesive responsible for a category Superpowers already owns and would lose Cohesive's substrate-shape distinction in the resulting prose-only plan.
...
`skills/implement-cohesively/SKILL.md` invokes `superpowers:writing-plans` once (Step 1) and `superpowers:executing-plans` once (Step 2) per implementation pass. There is no fallback. If Superpowers is not installed, the skill stops with a hard error.
...
Subskills' "Recommended next Cohesive skill" footer can recommend a Superpowers skill when the appropriate next step is outside Cohesive's substrate scope. Example: `validate-rewrite`'s "Approved" branch renders a decision matrix where one conditionally-rendered alternative is "Implement with Superpowers directly" via `superpowers:writing-plans` (the user accepts that implementation may drift from the rewrite; rendered when the rewrite is small enough that Cohesive's implement-cohesively flow would be ceremony, per `${CLAUDE_PLUGIN_ROOT}/skills/validate-rewrite/SKILL.md` §"Conditional alternatives"). The default option, however, is `cohesive:implement-cohesively` — the substrate-shaped path that composes both plugins.

`superpowers:finishing-a-development-branch` is recommended (not invoked) after `implement-cohesively` Implemented verdict.

### 4. Documentation of pass boundary

The README (§"Recommended companion") and this design doc state the pass boundary: Cohesive shapes the target *and the substrate-shape of how to get there*; Superpowers shapes the per-pass plan and TDD execution path. Users running both plugins are expected to invoke Cohesive first (substrate / design / spec rewrite / fresh-eyes review / implementation-orchestration), with Superpowers consumed inside `implement-cohesively`'s implementation pass and after `implement-cohesively`'s Implemented verdict (branch finishing). The router (`cohesively`) does not auto-invoke Superpowers directly — the `implement` route dispatches `cohesive:implement-cohesively`, which is the Cohesive skill that internally composes Superpowers per pass. Pass transitions remain user actions: the user picks the implementation route via the `validate-rewrite` Approved decision matrix, not by router auto-routing.
...
- **No Cohesive plan-writing skill.** `superpowers:writing-plans` produces the per-pass TDD plan inside `implement-cohesively`. Cohesive does not author TDD-shaped tasks itself; the thin intent paragraph passed to `writing-plans` is substrate-shape, and the translation to TDD-shape is Superpowers' job.
- **No Cohesive plan-execution skill.** `superpowers:executing-plans` consumes the per-pass plan and writes code with TDD discipline. Cohesive's `implement-cohesively` orchestrates the pass but never writes code itself.
...
- **A user installs only Cohesive and asks for implementation.** `cohesive:implement-cohesively` stops with a hard error and recommends installing Superpowers. The `validate-rewrite` Approved decision matrix's "Implement with Superpowers directly" alternative also requires Superpowers. The "Land specs first" alternative is available without Superpowers but defers the implementation question to a future session. The user-facing message is honest: Cohesive's substrate-shaped implementation needs Superpowers' per-pass plan and execution skills; without them, only the spec rewrite lands.
- **A user installs only Superpowers and asks for substrate review.** Out of Cohesive's hands; same reasoning.

## Alternatives considered

**Hard dependency on Superpowers across the entire plugin.** Rejected: makes Cohesive un-installable for users who want only substrate work (review, audit, design, rewrite). The chosen shape — Superpowers required for `implement-cohesively`'s implementation pass, optional for everything else — preserves optionality where the cost is bearable and accepts the dependency where the alternative is reinvention.

**Cohesive ships its own plan-writing and TDD execution skills.** Rejected: doubles the surface area, duplicates Superpowers' work, dilutes the substrate framing. The lighter Cohesive shape — owning *substrate-shaped intent paragraph* and *end-of-run dual reviewer dispatch* — is what the substrate framing actually demands.

**Tighter coupling — `cohesively` router auto-invoking Superpowers directly.** Rejected: would make pass transitions invisible to the user. The current model (user-driven pass transitions; the `implement` route dispatches `cohesive:implement-cohesively`, not Superpowers directly) is more legible. Superpowers is consumed inside `implement-cohesively`'s implementation pass, where the composition is structured and inspectable, rather than from the router where it would be hidden.
...
- If Superpowers renames or restructures `writing-plans` or `executing-plans`. `implement-cohesively`'s Hard constraint #2 names these skills explicitly; a rename without a Cohesive-side update breaks the implementation pass. Mitigation: Cohesive's CI does not currently grep Superpowers' skill set for compatibility; this is reviewer-judged.
...
- If a future Cohesive skill genuinely needs to write code without going through `superpowers:executing-plans` (e.g., a substrate-shaped code generator that produces test stubs from behavior matrices). At that point, the seam shifts further — Cohesive owns substrate-shaped code generation, Superpowers retains generic plan execution. v0.1 has no such case; the implementation pass's `superpowers:executing-plans` step covers the implementation surface for now.
...
- [`docs/substrate/invariants/IMPLEMENTATION_PLAN_COVERS_DELTA.md`](../invariants/IMPLEMENTATION_PLAN_COVERS_DELTA.md) — the named invariant that pins delta coverage across the implementation pass.
- [`docs/substrate/gotchas/large-delta-mega-plan.md`](../gotchas/large-delta-mega-plan.md) — the abandonment cliff the Step 1 size-budget gate mitigates.
