...
- **Re-decide** (Lock → Decide) — fires on `validate-rewrite`'s Approved verdict when the user reads the Architectural reflection and judges the locked design unsound, via the **Re-decide** option in the Approved trailer's `(other options)` disclosure. Agent-internal handoff: `validate-rewrite → brainstorm-design (Re-decide re-entry)` per [`handoffs.md`](${CLAUDE_PLUGIN_ROOT}/docs/substrate/architecture/handoffs.md).
