...
**Gates and reversals.** The user-facing flow has exactly **three gates** — Decide, Lock, Build — and they are *nodes*. A gate is a stop point where the user reads an outcome and decides whether to proceed. **Reversals** are *edges* between gates that run backward (counter to the Decide → Lock → Build forward flow); they are not gates themselves. Two reversals are named in the substrate today:

- **Substrate Drift** (Build → Lock) — fires on `implement-cohesively`'s Substrate Drift verdict when implementation introduces behavior the rewrite doesn't cover; the user re-enters Lock via `cohesive:rewrite-specs` to extend the rewrite (or revert the divergent code). Agent-internal handoff: `implement-cohesively → rewrite-specs (Substrate Drift)` per [`handoffs.md`](${CLAUDE_PLUGIN_ROOT}/docs/substrate/architecture/handoffs.md).
- **Re-decide** (Lock → Decide) — fires on `validate-rewrite`'s Approved verdict when the user reads the Architectural reflection and judges the locked design unsound, via the **Re-decide** alternative in the Approved trailer's `Other options` disclosure (conditionally rendered when the reflection identifies a structural concern, per [`skills/validate-rewrite/SKILL.md` §"Conditional alternatives"](${CLAUDE_PLUGIN_ROOT}/skills/validate-rewrite/SKILL.md)). Agent-internal handoff: `validate-rewrite → brainstorm-design (Re-decide re-entry)` per [`handoffs.md`](${CLAUDE_PLUGIN_ROOT}/docs/substrate/architecture/handoffs.md).

The user-facing chat-surface vocabulary uses the reversal name ("Substrate Drift" appears as a user-facing verdict label; "Re-decide" appears as the alternative name in the Approved trailer disclosure when its triggering condition fires). Reversal names sit alongside gate names in chat but the count of *gates* stays at three. A future contributor authoring a third reversal adds an edge, not a gate; gate vocabulary is the user-facing model's shape, reversal vocabulary is the user-facing model's escape routes.
